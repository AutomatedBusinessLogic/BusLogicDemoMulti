...
<%
	...
	MultiSessionFactory.setup(sessionId);
	
	// Any updates required?
	MultiUserRequestProcessor.processRequest(request);
	...
	if (currentCustName == null) { // No customer specified -- grab the first one by alphabetic order
		...
		List<Customer> customers = MultiSessionFactory.getSession(sessionId).createQuery("from Customer order by name").list();
		currentCustomer = customers.get(0);
		currentCustName = currentCustomer.getName();
	}
	else {
		currentCustomer = (Customer)MultiSessionFactory.getSession(sessionId).load(Customer.class, currentCustName);
	}
...
<body style="font-family: arial; padding: 10px;" onload="showMessage()">
	<script type="text/javascript">
		function update(custName, arg) {
			...
			window.location='multiUser.jsp?custName=' + escape(custName) + '&' + arg;
		}
	...
	<h3>Orders for customer :&nbsp;
		<span id="custNameSelect">
			<select name="custName" id="custNameControl" 
					onchange="update(this.options[this.selectedIndex].value, null)" style="font-size: 13pt;">
				<option <%= currentCustName.equals("Alpha and Sons") ? "selected" : "" %>>Alpha and Sons
				<option <%= currentCustName.equals("Bravo Hardware") ? "selected" : "" %>>Bravo Hardware
				<option <%= currentCustName.equals("Charlie's Construction") ? "selected" : "" %>>Charlie's Construction
				<option <%= currentCustName.equals("Delta Engineering") ? "selected" : "" %>>Delta Engineering
			</select>
		</span>
	</h3>
		...
		<tr>
			...
			<td style="font-size: 10pt; color: #777777; width: 500px; padding-left: 40px;">
				...
				<ul>
					...
					<li>Reassign an order to a different customer and check both customers' balances
				</ul>
	...
	<table cellpadding="3" cellspacing="0" id="ordersTable">
		...
		<thead style="background-color: #555555; color: #DDDDFF;">
						...
						%>
							<tr>
								...
								<td style="border: 1px solid #888888;">
									<select name="custName" id="productSelect<%= item.getLineitemId() %>" autocomplete="off"
										onchange="update(null, 'action=update&type=Lineitem&id=<%= item.getLineitemId() %>&att=product&value=' + this.options[this.selectedIndex].value)"
										style="font-size: 10pt;">
										<option value="1" <%= item.getProduct().getProductNumber() == 1L ? "selected" : "" %>>Drill - $315.00
										<option value="2" <%= item.getProduct().getProductNumber() == 2L ? "selected" : "" %>>Hammer - $10.00
										<option value="3" <%= item.getProduct().getProductNumber() == 3L ? "selected" : "" %>>Shovel - $25.00
									</select>
								</td>
			...
			<tr style="background-color: #CCCCCC;"><td colspan="6"><center>
				Reassign this order to customer: <select name="custName" 
								onchange="update(null, 'action=update&type=Order&id=<%= order.getOrderNumber() %>&att=customer&value=' + this.options[this.selectedIndex].value)"
								style="font-size: 10pt;">
							<option selected>- select a customer -
							<option>Alpha and Sons
							<option>Bravo Hardware
							<option>Charlie's Construction
							<option>Delta Engineering
						</select>
				&nbsp;&nbsp;&nbsp;
				<a href="multiUser.jsp?custName=<%= FormatUtil.escapeIdent(currentCustName) %>&action=insert&type=Lineitem&id=<%= order.getOrderNumber() %>">Create line item</a>
			</center></td></tr>
		...
		%>
		<tr>
			<td colspan="4"><a id="newOrderButton" href="multiUser.jsp?custName=<%= FormatUtil.escapeIdent(currentCustName) %>&action=insert&type=Order">Create new order</a></td>
			<td style="width: 60px; text-align: right;">
				<a href="multiUser.jsp?action=reloadData" onclick="return confirm('This will delete all data and reload the sample data set. Do you want to do that?')">Reload data</a>
			</td>
		</tr>
	</table>
</body>
...
<%
	MultiSessionFactory.commitTransaction(session.getId());
%>
